...
if (args[0] === "format:exports" || args[0] === "typecheck") {
	process.exit(0);
}
...
`;
	...
	for (const config of [".prettierignore", ".prettierrc.json", ".swift-format", ".swiftlint.yml"]) {
...
describe("check-staged", () => {
	...
	test("selects only the command groups required by staged extensions", (t) => {
		...
		assert.ok(commands.some((command) => command === "pnpm format:exports"));
